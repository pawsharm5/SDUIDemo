...
struct PSImageViewBuilder: UIComponentBuilder {
    ...
    func build(element: SubView) -> PSImageView {
        let configuration = PSImageViewConfig(identifier: element.identifier, image: Image(element.properties?.title ?? "info"), contentMode: .fill, size: CGSize(width: element.properties?.size?.width ?? 20 , height: element.properties?.size?.height ?? 20))
