...
struct PSLabelBuilder: UIComponentBuilder {
    ...
    func build(element: SubView) -> PSLabel {
        let configuration = PSLabelConfig(identifier: element.identifier ?? "", text: element.properties?.label ?? "", textColor: element.properties?.color ?? "", font: .body, padding: element.properties?.padding)
